resource "aws_cloudfront_distribution" "devopsaws" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A" 
      alias   = {
        name    = aws_cloudfront_distribution.devopsaws.domain_name
        zone_id = aws_cloudfront_distribution.devopsaws.hosted_zone_id
      }
    }
  ] 
}